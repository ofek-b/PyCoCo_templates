...
def main(SN_NAME):
    ...
    if np.count_nonzero(flux_V[t_V <= mjd_Vpeak] <= 0.8 * Vpeak) >= 2:
        ...
        max_MJD = mjd_Vpeak
    print('V band', 'First data:%.2f' % (min(t_V) - mjd_Vpeak),
          'Max point used for fitting:%.2f' % (max_MJD - mjd_Vpeak))
    ...
    for band in early_bands:
        ...
        if np.count_nonzero(flux_x[t_x <= mjd_xpeak] <= 0.8 * xpeak) >= 2:
            ...
            max_MJD = mjd_xpeak
        print(band, 'First data:%.2f' % (min(t_x) - mjd_xpeak),
              'Max point used for fitting:%.2f' % (max_MJD - mjd_xpeak))
