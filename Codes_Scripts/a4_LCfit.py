...
class SNPhotometryClass():
    ...
    def plot_GP_fit_phot(self, filt_list=None, save_fig=False, save_format='png', output_path_name=None):
        ...
        for f in filt_toplot:
            ...
            plt.errorbar(phot_f['MJD'], phot_f['Flux'], yerr=phot_f['Flux_err'], marker='.', linestyle='None', label=f)
        plt.legend(loc='best')
        plt.show()
        if save_fig:
            if output_path_name:
    ...
    def LCfit_withGP_xfilter(self, filt, minMJD=None, maxMJD=None):
        ...
        def ll(p):
            ...
            return -gp.log_likelihood(flux_norm, quiet=False)  # OFEK: changed from deprecated -gp.lnlikelihood(flux_norm, quiet=False)

        # gradient of the  liklihood for optimisation of the kernel size

        def grad_ll(p):
            gp.set_parameter_vector(p)
            return -gp.grad_log_likelihood(flux_norm, quiet=False)  # OFEK: changed from deprecated  -gp.grad_lnlikelihood(flux_norm, quiet=False)
        ...
        mjdT,flux_gp, flux_err_gp= mjdT[keep],flux_gp[keep], flux_err_gp[keep]  # OFEK: kick nans (from t==t_explosion)
...
for f in SN.avail_filters:
    ...
    a = plt.errorbar(mjd[sudo], flux[sudo], yerr=err_flux[sudo], fmt='o', mfc='None', mec=c, elinewidth=0.8, color=c,
