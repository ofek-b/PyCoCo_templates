...
def setPRIOR(SN_NAME, GP2DIM_Class, type_=None, PRIOR_file=None, PRIOR_folder=None):
    ...
    original_fit.dropna(subset=[Bessell_V_like], inplace=True)
    Vflux = original_fit[Bessell_V_like].values
    if Bessell_B_like in original_fit.columns:
        BVflux = original_fit[Bessell_V_like].values + original_fit[Bessell_B_like].values
    else:
        BVflux = original_fit[Bessell_V_like].values + original_fit[swift_B_like].values
...
def run_2DGP_GRID(GP2DIM_Class, y_data_nonan, y_data_nonan_err, x1_data_norm, x2_data_norm, \
    ...
    if prior:
        gp = george.GP(kernel2dim, mean=mean_model)  # , fit_mean=True, fit_white_noise=True)
    else:
    ...
    for j in range(int(len(extrap_mjds) / slot_size + 1)):
        mjd_normed_range = ((extrap_mjds[j * slot_size:(j + 1) * slot_size]) - GP2DIM_Class.grid_norm_info['offset2']) / \
    ...
    print('[', '*' * frac_tot_iteration, ' ' * (20 - frac_tot_iteration),
          ']' + '%i of %i' % (slot_size * (j + 1), slot_size * tot_iteration) + 'spec extrapolated')
...
def make_results_plots(GP2DIM_Class, x1_fill, x2_fill, mu_fill, std_fill):
    ...
    plt.yscale('log')
    plt.subplot(222)
    plt.title('from %.1f to %.1f' % (min(fit_wls[int(len_wls / 4):2 * int(len_wls / 4)] * norm1),
    ...
    plt.yscale('log')
    plt.subplot(223)
    plt.title('from %.1f to %.1f' % (min(fit_wls[2 * int(len_wls / 4):3 * int(len_wls / 4)] * norm1),
...
def transform_back_andPlot(GP2DIM_Class, x1_fill, x2_fill, mu_fill, std_fill, y_data_nonan):
    ...
    plt.xlim(1600, 11000)
    for b in GP2DIM_Class.avail_filters:
        plt.vlines((GP2DIM_Class.lam_eff(b)), 0, 1., linestyle='--', lw=4, label=b,
                   color='m')  # OFEK: changed color from PyCoCo_info.color_dict[b]
...
def save_plots_files(GP2DIM_Class, list_mjds_tot, y_data_conv, x1_fill, x2_fill, mu_fill_conv, std_fill_conv):
    ...
    for j in range(len(list_mjds_tot)):
        ...
        if (mj in list_mjds_spec) & (GP2DIM_Class.mode == 'extend_spectra'):
            ...
            plt.plot(wls, smooth_ext_spec + (a + 1) * scale, label='Extrapolated %i' % (mj - offset2), lw=0.8,
                     color='r')
