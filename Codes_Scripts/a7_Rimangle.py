...
def main(SN_NAME):
    ...
    class ReMangle_SingleSpectrumClass():
        ...
        def get_spec_list(self, verbose=False):
            ...
            onlyfiles = [f for f in os.listdir(extended_files_path) if
                         os.path.isfile(os.path.join(extended_files_path, f))
        ...
        def mangle_iteration_function(self):
            ...
            ax1.fill_between(self.ext_spec['wls'], self.mangling_mask_FINAL - mask_err,
                             self.mangling_mask_FINAL + mask_err,
            ...
            if ('FL' in self.spec_file) | ('SNF' in self.spec_file):
                ax2.errorbar(self.mangled_spec[last_iter]['wls'], self.mangled_spec[last_iter]['flux'], lw=0.7,
                             color='k',
        ...
        def save_mangled_spectrum(self):
            ...
            if 'FL' in self.spec_file:
                fout = open(
                    self.mangledspec_path + self.spec_file.replace('_spec_extended_FL.txt', '_REmangled_spec_FL.txt'),
                    'w')
        ...
        def save_FINAL_spectrum(self):
            ...
            for path_fold in ['', '/HostNotCorr/', '/as_observed/']:
                ...
                if 'FL' in self.spec_file:
                    ...
                    fout = open(path + self.spec_file.replace('_spec_extended_SMOOTH.txt', '_FINAL_spec_SMOOTH.txt'),
                                'w')
                elif 'SNF' in self.spec_file:
