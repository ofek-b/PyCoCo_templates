...
def main(SN_NAME):
    ...
    def savitzky_golay(y, window_size, order, deriv=0, rate=1):
       ...
       """
        ...
        b_deriv = np.mat(
            [[(i + 1) * k ** (i) for i in order_range_deriv] for k in range(-half_window, half_window + 1)])
    ...
    class Spectra_xTarget():
        ...
        def clipping(self, sp_file_name):
            ...
            while np.any(mask_clip) & (it < 100):
                ...
                # OFEK: only take indices in the axis, otherwise raises error:
                df.drop(labels=np.unique([ridx for ridx in reject_index if ridx in df.axes[0]]), axis=0, inplace=True)
